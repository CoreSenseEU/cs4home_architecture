 ...
 * instance.
 */
Afferent::Afferent(const std::string &name,
...
/**
 * @brief Configures the Afferent by creating subscribers for each specified
 * topic.
 *
 * This method retrieves the topic names from the parameter server and attempts
 * to create a subscription for each topic to receive messages.
 *
 * @return True if all subscriptions are created successfully.
 */
bool Afferent::configure() {
