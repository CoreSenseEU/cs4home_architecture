 ...
 * headers and republishing them. Uses ROS 2 lifecycle and callback functions
...
public:
  ...
  /**
   * @brief Constructs an ImageFilterCB object and initializes the parent
   * lifecycle node.
  ...
  /**
   * @brief Processes incoming serialized image messages, applies a simple
   * transformation, and republishes them.
   *
   ...
   * efferent component.
   *
   * @param msg shared pointer to the serialized incoming image message.
   */
  void process_in_image(std::shared_ptr<rclcpp::SerializedMessage> msg) {
    auto image_msg =
   ...
   *
   * Sets the afferent component mode to `CALLBACK`, binding the
   * `process_in_image` method to handle incoming messages.
   *
...
private:
  rclcpp::TimerBase::SharedPtr
      timer_; /**< Timer used for periodic operations if needed. */
};
