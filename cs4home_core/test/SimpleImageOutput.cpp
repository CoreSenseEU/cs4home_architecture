...
public:
  ...
  explicit SimpleImageOutput(rclcpp_lifecycle::LifecycleNode::SharedPtr parent)
    ...
    parent_->declare_parameter("simple_image_output.topics",
                               output_topic_names_);
  }
  ...
  bool configure() {
    ...
    for (size_t i = 0; i < output_topic_names_.size(); i++) {
      if (create_publisher(output_topic_names_[i], "sensor_msgs/msg/Image")) {
        RCLCPP_DEBUG(parent_->get_logger(),
                     "[SimpleImageOutput] created publisher to [%s, "
                     "sensor_msgs/msg/Image]",
                     output_topic_names_[i].c_str());
      } else {
        RCLCPP_WARN(parent_->get_logger(),
                    "[SimpleImageOutput] Couldn't create publisher to [%s, "
                    "sensor_msgs/msg/Image]",
                    output_topic_names_[i].c_str());
      }
    }
   ...
   * `sensor_msgs::msg::Image`.
   */
  void publish_image(std::shared_ptr<sensor_msgs::msg::Image> msg) {
    publish(std::move(msg));
  }

private:
  std::vector<std::string>
      output_topic_names_; /**< List of output topics to publish images. */
};
