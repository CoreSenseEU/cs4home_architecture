...
TEST(master_test, master_creation) {
  ...
  auto cm2 =
      cs4home_core::CognitiveModule::make_shared("cognitive_module_2", options);
  ...
  master->trigger_transition(
      lifecycle_msgs::msg::Transition::TRANSITION_CONFIGURE);
  cm1->trigger_transition(
      lifecycle_msgs::msg::Transition::TRANSITION_CONFIGURE);
  cm2->trigger_transition(
      lifecycle_msgs::msg::Transition::TRANSITION_CONFIGURE);
  ASSERT_EQ(master->get_current_state().id(),
            lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE);
  ASSERT_EQ(cm1->get_current_state().id(),
            lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE);
  ASSERT_EQ(cm2->get_current_state().id(),
            lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE);
  ...
  ASSERT_EQ(flow_1_cms, std::vector<std::string>(
                            {"cognitive_module_1", "cognitive_module_2"}));
 ...
 *
 * This test checks that `Flow` instances are created correctly with the
 * expected sequence of nodes, ensuring that the get_nodes() function returns
 * the correct vector of node names.
 */
 ...
 *
 * Initializes GoogleTest and ROS 2, then runs all tests defined in the
 * executable.
 *
