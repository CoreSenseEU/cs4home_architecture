...
public:
   ...
   * @return A shared pointer to the next message, or nullptr if the queue is
   * empty.
   */
  template <class MessageT> std::shared_ptr<MessageT> get_msg() {
    if (msg_queue_.empty()) {
      return {};
    }

    std::shared_ptr<rclcpp::SerializedMessage> msg =
        std::move(msg_queue_.front());
    msg_queue_.pop();
...
protected:
  ...
  std::queue<std::shared_ptr<rclcpp::SerializedMessage>> msg_queue_;
